...
namespace OpenGL_Core
{
    ...
    void ProcessNode(ModelImporter& importer, const aiNode* node, const aiScene* scene, Transform* parent)
    {
        auto objPtr = make_unique<GameObject>(node->mName.C_Str());
        auto obj = objPtr.get();
        importer.gameObjects.push_back(move(objPtr));
        ...
        {
            ProcessNode(importer, node->mChildren[i], scene, &obj->GetTransform());
        }
    }
    void ProcessScene(ModelImporter& importer, const aiScene* scene)
    {
        ...
        {
            importer.materials.push_back(move(ProcessMaterial(scene->mMaterials[i])));
        }

        for (unsigned int i = 0; i < scene->mNumMeshes; i++)
        {
            importer.meshs.push_back(move(ProcessMesh(scene->mMeshes[i])));
        }

        ProcessNode(importer, scene->mRootNode, scene, nullptr);
    }

    unique_ptr<ModelImporter> ModelImporter::ModelLoad(const string& path, unsigned int postProcessSetps)
    {
        //Assimp::Importer i;
...
//#endif // GLM_FORCE_LEFT_HANDED
        ...
        auto assimp = unique_ptr<ModelImporter>(new ModelImporter());
        ProcessScene(*assimp, scene);
        return assimp;
    }

    void ModelImporter::AssingToScene(Scene& scene, ModelImporter& assimp)
    {

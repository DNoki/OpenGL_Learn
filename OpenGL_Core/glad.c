/*
    ...
    OpenGL loader generated by glad 0.1.29 on Tue Mar  5 17:06:06 2019.
...
static int get_exts(void) {
    ...
    if(max_loaded_major < 3) {
        ...
        if (num_exts_i > 0) {
            exts_i = (char **)malloc((unsigned int)num_exts_i * (sizeof *exts_i));
        }
        ...
        for(index = 0; index < (unsigned)num_exts_i; index++) {
            ...
            unsigned int len = strlen(gl_str_tmp);
...
static void find_coreGL(void) {
    ...
    for (i = 0;  prefixes[i];  i++) {
        const unsigned int length = strlen(prefixes[i]);
